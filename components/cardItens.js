...
export default function CardItens({offer, image, title, original, link}, key) {
    ...
    return (
        <div className = {`${styles.card} ${caveat.variable}`}>
            ...
            <Link href = {link} className = {styles.algnbtn}><button type = "button" className = {`btn btn-primary ${styles.botaoComprar}`} style = {{marginTop: offer === 0 ? "14px" : null}}>Comprar</button></Link>
        </div>
    )
}